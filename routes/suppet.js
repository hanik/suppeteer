...
router.post('/runstep', (req, res, next) => {
    ...
    let steps = store.get('current').steps
    console.log(steps[store.get('current').stepCount + 1])

    return (async () => {
        await puppethelper.runStep(steps[store.get('current').stepCount + 1], currentPage)
    })()
